...
def get_entry_by_id(entryid):
    con = get_database_connection()
    cur = con.cursor()
    cur.execute(DB_ENTRY_ID_QUERY, [str(entryid)])
    keys = ('id', 'title', 'text', 'created')
    return dict(zip(keys, cur.fetchone()))


def update_entry_in_db(entryid, title, text):
    if not title or not text:
        raise ValueError("Title and text required for writing an entry")
    con = get_database_connection()
    cur = con.cursor()
...
def delete_entry_in_db(entryid):
    con = get_database_connection()
    cur = con.cursor()
    cur.execute(DB_DELETE_ENTRY, [entryid])
...
def update_entry(entryid):
    try:
        update_entry_in_db(entryid, request.form['title'],
                           request.form['text'])
    except psycopg2.Error:
        # this will catch any errors generated by the database
        abort(500)
